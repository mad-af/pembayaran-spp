<template>
  <Drawer v-bind="{ items:routers, user:user, page:'Sumbangan Pembinaan Pendidikan' }">
    <SimpleCard v-bind="{ theme:'md-primary', title:'Total SPP', price:'10', icon:'paid' }" />

    <div class="table">
      <md-card>
        <md-card-header>
          <md-card-header-text class="md-layout md-alignment-center-space-between">
            <div class="md-title md-layout-itemt">Tabel SPP</div>
            <div class="md-layout-itemt">
              <Modal v-bind="{ title:'Formulir Tambah SPP', button:'Tambah SPP', theme:'md-primary md-raised' }">
                <FormSPP />
              </Modal>
            </div>
          </md-card-header-text>
...
<script>
  ...
  export default {
    components: {
      ...
      TableSPP: components.tableSPP,
      Modal: components.modal,
      FormSPP: components.formSPP
    },
    data: function () {
      return {
        routers : routers,
