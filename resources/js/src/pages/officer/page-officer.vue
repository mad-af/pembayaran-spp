<template>
  <Drawer v-bind="{ items:routers, user:user, page:'Petugas' }">
    <SimpleCard v-bind="{ theme:'md-primary', title:'Total Petugas', price:'10', icon:'supervisor_account' }" />

    <div class="table">
      <md-card>
        <md-card-header>
          <md-card-header-text class="md-layout md-alignment-center-space-between">
            <div class="md-title md-layout-itemt">Tabel Petugas</div>
            <div class="md-layout-itemt">
              <Modal v-bind="{ title:'Formulir Tambah Petugas', button:'Tambah Petugas', theme:'md-primary md-raised' }">
                <FormOfficer />
              </Modal>
            </div>
          </md-card-header-text>
      ...
      <TableOfficer v-bind="{ data: students }" />
    </div>
    <br><br>
  </Drawer>
</template>
...
<script>
  ...
  export default {
    components: {
      ...
      TableOfficer: components.tableOfficer,
      Modal: components.modal,
      FormOfficer: components.formOfficer
    },
    data: function () {
      return {
        routers : routers,
